...
create_master_tables <- function(database_path,
  ...
  if (file.exists(distances_file)) {
    ...
    # df_all_novel_raw <- readRDS(file = "/home/sruiz/PROJECTS/splicing-project/splicing-recount3-projects/df_all_distances_pairings_raw.rds")
    # 
    ...
    # 
    # 
    # aux_introns_tidy <- readRDS(file = "/home/sruiz/PROJECTS/splicing-project/splicing-recount3-projects/df_all_introns.rds")  %>%
  ...
  saveRDS(object = df_introns_introverse,
          file = "/home/sruiz/PROJECTS/splicing-project-recount3/database/df_all_introns_introverse.rds")
  ...
  saveRDS(object = df_introns_introverse_tidy,
          file = "/home/sruiz/PROJECTS/splicing-project-recount3/database/df_all_introns_introverse_tidy.rds")
  ...
  create_gene_table(df_introns_introverse_tidy %>% unnest(gene_id) %>% distinct(gene_id))
  
  ######################################
  ## INTRONS - ADD THE GENE FOREING KEY
  ######################################

  print(paste0(Sys.time(), " --> adding GENE foreing key to the introns..."))
  ...
  df_all_introns <- df_all_introns %>%
    ...
    dplyr::relocate(c(ref_ss5score, ref_ss3score), .before = gene_id ) 
  ...
  if (any(df_all_introns_tidy_final$gene_id %>% is.na())) {
...
create_gene_table <- function(gene_ids) {
  ...
  for (db in SRA_projects) {
    ...
    for (cluster in clusters) { 
      ...
      if ( file.exists(paste0(base_folder, "results/pipeline3/distances/", 
                             cluster, "/v", gtf_version, "/", cluster, "_raw_distances_tidy.rds")) ) {
        
        
        ## BASE DATA -----------------------------------------------------------
        
        ## Load split read counts
        split_read_counts <- readRDS(file = paste0(base_folder, "/results/base_data/", cluster, "/",
                                                   db, "_", cluster, "_split_read_counts_", gtf_version, ".rds"))
        
        ## Load samples
        samples <- readRDS(file = paste0(base_folder, "/results/base_data/", cluster, 
                                         "/", db, "_", cluster,  "_samples.rds"))
        
        
        
        ## LOAD INTRONS AND NOVEL JUNCTIONS ------------------------------------
        df_cluster_distances <- readRDS(file = paste0(base_folder, "results/pipeline3/distances/", 
                                                      cluster, "/v", gtf_version, "/", cluster, "_raw_distances_tidy.rds")) %>% as_tibble()
        ...
        df_intron_novel_merged <- df_novel_merged %>%
          dplyr::select(-c(seqnames, start, end, strand)) %>%
          inner_join(y = df_introns_merged %>% dplyr::select(-c(seqnames, start, end, strand)),
                     by = "ref_junID")
        ...
        if (identical(df_all_misspliced$novel_coordinates %>% sort(), 
          ...
          ## TYPE 'NONE'
          introns_never <- readRDS(file = paste0(base_folder, "results/pipeline3/distances/", 
          ...
          if (any(str_detect(split_read_counts$junID,pattern = "\\*"))) {
