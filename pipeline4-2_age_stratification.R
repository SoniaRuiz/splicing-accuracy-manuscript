...
age_stratification_plot_distances <- function(df = NULL,
  ...
  df_age_distances <- map_df((df_metadata$SRA_project %>% unique())[3], function(project_id) {
  ...
  df_age_distances <- df_age_distances %>%
  ...
  df_age_distances <- df_age_distances %>%
    ...
    mutate(sample_type = factor(sample_type, levels = age_groups))
  
  
  
  plot_distances <- ggplot(data = df_age_distances) + 
  ...
  file_name <- paste0("/home/sruiz/PROJECTS/splicing-project-recount3/paper_figures/panel4_age_distances_muscle.svg")
