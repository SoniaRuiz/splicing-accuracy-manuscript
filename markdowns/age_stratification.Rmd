...
age_stratification_plot_MSR(df = df_age_groups_intron_tidy,
...
GO Enrichment of genes that contain introns whose mis-splicing at the donor increases with age.
...
## MSR_D UNIQUE TO BRAIN

GO Enrichment of genes UNIQUE to brain that contain introns whose mis-splicing at the donor increases with age.

```{r GO_MRSD_uniquebrain, warning = FALSE, message = F, echo = FALSE}
...
only_brain_ref <- setdiff(brain_df_age_groups_intron_tidy$ref_junID, 
                          c(muscle_df_age_groups_intron_tidy$ref_junID,
                            blood_df_age_groups_intron_tidy$ref_junID))

df_MSRD <- df_age_groups_intron_tidy %>%
  filter(ref_junID %in% only_brain_ref) %>%
  select(ref_junID,
         sample_type,
         MSR_D = ref_missplicing_ratio_tissue_ND,
         gene_name) %>%
...
GO Enrichment of genes that contain introns whose mis-splicing at acceptor increase with age
...
## MSR_A UNIQUE TO BRAIN

GO Enrichment of genes UNIQUE to brain that contain introns whose mis-splicing at the acceptor increases with age.

```{r GO_MRSA_uniquebrain, warning = FALSE, message = F, echo = FALSE}
