...
This document contains the age analysis performed using RNA-seq data from the GTEx v8 project and processed by recount2.
...
* 20-29: only contains samples from the age group "20-29".
* 30-39: only contains samples from the age group "30-39".
* 40-49: only contains samples from the age group "40-49".



# Stats table {.tabset .tabset-fade .tabset-pills}

Samples have been displayed after grouping them by different columns such as 'age', 'sex', 'RIN' and 'body region'.
...
To account for these differences, we calculated the proportion of splicing noise lying at each distance point and per age supergroups. 

This proportion case calculated by dividing the total number of novel events located at a particular distance point by the total number of novel junctions available within the window of -60/+60bp distances to the reference intron.

<!-- For instance, if the "20-39" age supergroup presented a total number of 1,000 novel events located between -60/+60 bp of their referece introns, and there were 100 novel events located at positon +3bp, the proportion of splicing noise generated at +3bp corresponded to 100/1000 = 0.1 (i.e. 10%). -->

**Results show a change in the distribution of splicing noise with age occurring in close distance (-60/+60 bp) to the reference introns.**
...
print("MSR_D is smaller in '20-39' y/o samples than in '40-59' y/o samples:")
wilcox.test(x = df_wilcoxon$`20-39`,
            y = df_wilcoxon$`40-59`,
            alternative = "less") %>% print()

print("MSR_D is smaller in '20-39' y/o samples than in '60-79' y/o samples:")
wilcox.test(x = df_wilcoxon$`20-39`,
            y = df_wilcoxon$`60-79`,
            alternative = "less")%>% print()

print("MSR_D IS NOT SMALLER in '40-59' y/o samples than in '60-79' y/o samples:")
...
<!-- To statistically test whether overlapping introns across age supergroups presented smaller MSR_A values within the 20-39 supergroup than in the 40-59 and smaller than in 60-79, we used the Wilcoxon test. -->

<!-- Results from this analysis showed that *the median value corresponding to the distribution of MSR_A values of the 116,847 introns that were common across age supergroups significantly increased with age*: -->
...
print("MSR_A in introns from '20-39' y/o < MSR_A in samples from '40-59' y/o:")
wilcox.test(x = df_wilcoxon_MSR_A$`20-39`,
            y = df_wilcoxon_MSR_A$`40-59`,
            alternative = "less") %>% print()

print("MSR_A in introns from '20-39' y/o < MSR_A in samples from `60-79` y/o:")
wilcox.test(x = df_wilcoxon_MSR_A$`20-39`,
            y = df_wilcoxon_MSR_A$`60-79`,
            alternative = "less")%>% print()

print("MSR_A in introns from `40-59` y/o IS NOT < MSR_A in samples from `60-79` y/o:")
...
* 1. Calculate the TPM expression of the core spliceosomal RBPs from GTEX v8 data, which is calculated by:
...
* 2. Get the best covariates to correct for:
    * 1. PCA of the 'TPM' values across samples from the spliceosomal RBPs
...
```{r U2AF2_tpm, warning=FALSE}
  ...
  wilcox.test(x = tpm_20_39$ENSG00000063244, 
              y = tpm_40_59$ENSG00000063244, 
              correct = T,
              alternative = "greater") %>% print()
  

  wilcox.test(x = tpm_20_39$ENSG00000063244, 
              y = tpm_60_79$ENSG00000063244, 
              alternative = "greater") %>% print()
  

  wilcox.test(x = tpm_40_59$ENSG00000063244, 
              y = tpm_60_79$ENSG00000063244,
              alternative = "greater") %>% print()
...
<!-- There wasn't any intron never mis-spliced at both donor and acceptor across age groups. -->

<!-- ## GO Enrichment - Never mis-spliced at MSR_D -->

We obtained the gene enrichment of those genes containing introns whose MSR_D values explicitly increased with age.

The approach followed consisted of:

* Obtain all common introns whose MSR_D value is always greater in "20-39" than in "40-59" than in "60-79". 
* Obtain their gene name.
* From this dataset of genes, remove any overlapping gene that contain introns whose MSR_D value is always smaller in "20-39" than in "40-59" than in "60-79".
* Perform their Go & KEGG Enrichment.
* The custom background used was formed by all genes from all common introns across age supergroups.


## MSR_D Increasing with age

```{r GO_never_MRSD, warning=FALSE, message=F}
...
```{r GO_MRSA, warning=FALSE, message=F}
