...
project_clusters <-  readRDS(file = paste0("/home/sruiz/PROJECTS/splicing-project/splicing-recount3-projects/", 
...
Blood was chosen as the reference tissue. In addition, each analysis provides with a summary table containing the results obtained for each GTEx v8 tissue.
...
for (cluster in project_clusters[1]) { # tissue <- gtex_tissues[11]
...
for (cluster in project_clusters[1]) {
  
  cat('\n') 
  cat("####", cluster, "\n")
  
  folder_name <- paste0("/home/sruiz/PROJECTS/splicing-project/splicing-recount3-projects/", 
                        project_id, "/results/pipeline3/missplicing-ratio/", cluster, "/v", gtf_version, "/")

  plot_distances_modulo(cluster = cluster,
...
for (tissue in project_clusters[1]) {
...
To be able to statistically assess which genomic features could be impacting on the generation of splicing noise, we included diverse predictors encompassing diverse genomic levels. We used (1) features at the gene level such as the median TPM level of expression across samples of the gene and its length; (2) features at the transcript level, such as the number of transcripts of the gene and the transcript biotype; and (3) features at the intron level like the MaxEntScan scores of the 5’ss and 3’ss, the mean conservation and CTDS scores of the proximal intronic secuences and the intron length. 
...
for (tissue in project_clusters[1]) {
  ...
  folder_name <- paste0("/home/sruiz/PROJECTS/splicing-project/splicing-recount3-projects/", project_id,
                        "/results/pipeline3/missplicing-ratio/", 
                        tissue, "/v105/")
...
We then obtained the gene enrichment from the subset of genes corresponding to each intron category.

Results from this analysis showed that all four unique subsets of genes did not present any relevant enrichment, **reinforcing the hypothesis that mis-splicing is not a product of a faulty genomic sequence. Instead, it might be due to an inadequate level of expression of the RBPs responsible for recognising the canonical splicing signals that are required for correct splicing.**

#### Stats

Each category of unique reference introns belong to a unique set of genes. In this sense, any overlapping gene across different intronic categories were discarded.
...
This analysis was performed using a custom background of genes formed by the full set of genes corresponding to all common introns across tissues included in this analysis. The inputed dataset of genes were not ordered and the p-values returned by the GO Enrichment analysis were corrected using the Bonferroni method.
...
```{r GO_both, message=FALSE, echo=FALSE}
  ...
  if (!is.null(gene_enrichment_both$result)) {
    ...
    gene_enrichment_both$result %>% 
      select(p_value, term_name, source) %>%
      dplyr::mutate(p_value = p_value %>% as.double()) %>%
...
GO Enrichment analysis (gprofiler2) performed only over the unique genes from introns that can be mis-spliced at 'donor' splice sites across all brain tissues.

This analysis has been performed using a custom background of genes formed by all genes from the dataset of all common unique introns found across all brain regions. The inputed genes were not ordered and the p-values have been Bonferroni corrected.
...
GO Enrichment analysis (gprofiler2) performed only over the unique genes from introns that can be mis-spliced at 'acceptor' splice sites across all brain tissues.
...
GO Enrichment analysis (gprofiler2) performed only over the unique genes from introns that are never mis-spliced across brain tissues.
