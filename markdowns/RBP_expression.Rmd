...
```{r lm_comparison, echo=FALSE}
  ...
  df_tissues %>% 
    filter(pval <= 0.05) %>%
    dplyr::select(Estimate, name, tissue) %>%
    spread(key = tissue, value = Estimate) %>%
    dplyr::rename(RBP = name) %>%
    DT::datatable()
  

```

The density plot below contains the same information than the table above. 

Results from this plot show that BRAIN and BLOOD tissues present a more negative distribution of Beta values than MUSCLE. This suggests that, in BRAIN and BLOOD tissues, age produces a higher effect in the reduction of the level of expression of spliceosomal RBPs compared to MUSCLE.

```{r lm_comparison_densityplot, echo=FALSE}
